document.ngapp.value("translations", {
	"activities": {
		"cycling": {
			"sv": "Cykling",
			"en": "Cycling"
		},
		"mountain_biking": {
			"sv": "Terrängcykling",
			"en": "Mountain Biking"
		},
		"spinning": {
			"sv": "Spinning",
			"en": "Spinning"
		},
		"arc_trainer": {
			"sv": "Arc Trainer",
			"en": "Arc Trainer"
		},
		"elliptical": {
			"sv": "Elliptical",
			"en": "Elliptical"
		},
		"rowing": {
			"sv": "Rodd",
			"en": "Rowing"
		},
		"stair_walking_machine": {
			"sv": "Trappmaskin",
			"en": "Stair walking machine"
		},
		"aerobics": {
			"sv": "Aerobics",
			"en": "Aerobics"
		},
		"bootcamp": {
			"sv": "Träningsläger",
			"en": "BootCamp"
		},
		"circuit_training": {
			"sv": "Cirkelträning",
			"en": "Circuit Training"
		},
		"core_strength_training": {
			"sv": "Core Styrketräning",
			"en": "Core Strength Training"
		},
		"crossfit®": {
			"sv": "CrossFit®",
			"en": "CrossFit®"
		},
		"dance": {
			"sv": "Dans",
			"en": "Dance"
		},
		"group_workout": {
			"sv": "Gruppträning",
			"en": "Group Workout"
		},
		"pole_dance": {
			"sv": "Pole Dance",
			"en": "Pole Dance"
		},
		"zumba®": {
			"sv": "Zumba®",
			"en": "Zumba®"
		},
		"barre": {
			"sv": "Barr",
			"en": "Barre"
		},
		"gymnastics": {
			"sv": "Gympa",
			"en": "Gymnastics"
		},
		"boxing": {
			"sv": "Boxning",
			"en": "Boxing"
		},
		"mma": {
			"sv": "MMA",
			"en": "MMA"
		},
		"meditation": {
			"sv": "Meditation",
			"en": "Meditation"
		},
		"pilates": {
			"sv": "Pilates",
			"en": "Pilates"
		},
		"yoga": {
			"sv": "Yoga",
			"en": "Yoga"
		},
		"hiking": {
			"sv": "Vandring",
			"en": "Hiking"
		},
		"nordic_walking": {
			"sv": "Stavgång",
			"en": "Nordic Walking"
		},
		"power_walking": {
			"sv": "Power Walking",
			"en": "Power Walking"
		},
		"running": {
			"sv": "Löpning",
			"en": "Running"
		},
		"walking": {
			"sv": "Promenad",
			"en": "Walking"
		},
		"roller_skating": {
			"sv": "Rullskridskor",
			"en": "Roller Skating"
		},
		"ice_skating": {
			"sv": "Skridskor",
			"en": "Ice Skating"
		},
		"cross_country_skiing": {
			"sv": "Längdskidor",
			"en": "Cross Country Skiing"
		},
		"downhill_skiing": {
			"sv": "Utförsåkning",
			"en": "Downhill Skiing"
		},
		"roller_skiing": {
			"sv": "Rullskidor",
			"en": "Roller Skiing"
		},
		"slalom": {
			"sv": "Slalom",
			"en": "Slalom"
		},
		"snowboarding": {
			"sv": "Snowboarding",
			"en": "Snowboarding"
		},
		"strength_training": {
			"sv": "Styrketräning",
			"en": "Strength Training"
		},
		"swimming": {
			"sv": "Simmning",
			"en": "Swimming"
		},
		"other": {
			"sv": "Annat",
			"en": "Other"
		}
	},
	"groups": {
		"biking": {
			"sv": "Cykling",
			"en": "Biking"
		},
		"cardiac_training": {
			"sv": "Konditionsträning",
			"en": "Cardiac Training"
		},
		"group_training": {
			"sv": "Gruppträning",
			"en": "Group Training"
		},
		"gymnastics": {
			"sv": "Gymnastik",
			"en": "Gymnastics"
		},
		"marshal_arts": {
			"sv": "Kampsporter",
			"en": "Marshal Arts"
		},
		"meditative_training": {
			"sv": "Meditativ Träning",
			"en": "Meditative Training"
		},
		"running": {
			"sv": "Löpning",
			"en": "Running"
		},
		"walking": {
			"sv": "Promenader",
			"en": "Walking"
		},
		"skating": {
			"sv": "Skridskor",
			"en": "Skating"
		},
		"winter_sports": {
			"sv": "Vintersporter",
			"en": "Winter sports"
		},
		"others": {
			"sv": "Annat",
			"en": "Others"
		}
	},
	"general": {
		"logo": {
			"sv": "Hälsomått",
			"en": "Healthmeasure"
		}
	},
	"menu": {
		"measure": {
			"sv": "Mät",
			"en": "Measure"
		},
		"weigh": {
			"sv": "Väg",
			"en": "Weigh"
		},
		"exercise": {
			"sv": "Träna",
			"en": "Exercise"
		},
		"report": {
			"sv": "Statistik",
			"en": "Statistics"
		},
		"settings": {
			"sv": "Inställningar",
			"en": "Settings"
		},
		"feedback": {
			"sv": "Återkoppling",
			"en": "Feedback"
		}
	},
	"page": {
		"measure": {
			"enter_measurements": {
				"sv": "Ange kroppsdelarnas omkrets i mm",
				"en": "Enter body measurements in mm"
			},
			"form": {
				"biceps": {
					"label": {
						"sv": "Överarm",
						"en": "Biceps"
					}
				},
				"left_biceps": {
					"label": {
						"sv": "Vänster överarm",
						"en": "Left Biceps"
					}
				},
				"right_biceps": {
					"label": {
						"sv": "Höger överarm",
						"en": "Right Biceps"
					}
				},
				"bust": {
					"label": {
						"sv": "Byst",
						"en": "Bust"
					}
				},
				"tummy": {
					"label": {
						"sv": "Mage",
						"en": "Tummy"
					}
				},
				"butt": {
					"label": {
						"sv": "Stuss",
						"en": "Butt"
					}
				},
				"thigh": {
					"label": {
						"sv": "Lår",
						"en": "Thigh"
					}
				},
				"left_thigh": {
					"label": {
						"sv": "Vänster lår",
						"en": "Left Thigh"
					}
				},
				"right_thigh": {
					"label": {
						"sv": "Höger lår",
						"en": "Right Thigh"
					}
				},
				"calf": {
					"label": {
						"sv": "Vad",
						"en": "Calf"
					}
				},
				"left_calf": {
					"label": {
						"sv": "Vänster vad",
						"en": "Left Calf"
					}
				},
				"right_calf": {
					"label": {
						"sv": "Höger vad",
						"en": "Right Calf"
					}
				},
			},
			"error": {
				"empty_form": {
					"sv": "Du har inte angett några mått",
					"en": "You haven't entered any measurements"
				},
				"left_bicep": {
					"required": {
						"sv": "Vänster överarm måste mätas.",
						"en": "Your vänster biceps is required."
					},
					"minlength": {
						"sv": "Mått på vänster överarm borde vara minst 2 siffror.",
						"en": "Your vänster biceps is required to be at least 2 digits."
					},
					"maxlength": {
						"sv": "Mått på vänster överarm får max vara 4 siffror.",
						"en": "Your vänster biceps cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt överarmsmått, 2 till 4 siffror krävs.",
						"en": "That is not a valid vänster biceps measurment, only 2-4 digits allowed."
					}
				},
				"right_bicep": {
					"required": {
						"sv": "Höger överarm måste mätas.",
						"en": "Your right biceps is required."
					},
					"minlength": {
						"sv": "Mått på höger överarm borde vara minst 2 siffror.",
						"en": "Your right biceps is required to be at least 2 digits."
					},
					"maxlength": {
						"sv": "Mått på höger överarm får max vara 4 siffror.",
						"en": "Your right biceps cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt överarmsmått, 2 till 4 siffror krävs.",
						"en": "That is not a valid right biceps measurment, only 2-4 digits allowed."
					}
				},
				"bust": {
					"required": {
						"sv": "Du måste mäta bystens omkrets.",
						"en": "The circumference of your bust is required."
					},
					"minlength": {
						"sv": "Mått på bysten borde vara minst 3 siffror.",
						"en": "Your bust is required to be at least 3 digits."
					},
					"maxlength": {
						"sv": "Mått på bysten får max vara 4 siffror.",
						"en": "Your bust cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt bystmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid bust measurment, only 3-4 digits allowed."
					}
				},
				"tummy": {
					"required": {
						"sv": "Du måste mäta magens omkrets.",
						"en": "The circumference of your tummy is required."
					},
					"minlength": {
						"sv": "Magens omkrets borde vara minst 3 siffror.",
						"en": "Your tummy is required to be at least 3 digits."
					},
					"maxlength": {
						"sv": "Magens omkrets får max vara 4 siffror.",
						"en": "Your tummy cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt magmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid tummy measurment, only 3-4 digits allowed."
					}
				},
				"butt": {
					"required": {
						"sv": "Du måste mäta stussens omkrets.",
						"en": "The circumference of your butt is required."
					},
					"minlength": {
						"sv": "Stussens omkrets borde vara minst 3 siffror.",
						"en": "Your Butt is required to be at least 3 digits."
					},
					"maxlength": {
						"sv": "Stussens omkrets får max vara 4 siffror.",
						"en": "Your Butt cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt stussmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid Butt measurment, only 3-4 digits allowed."
					}
				},
				"left_thigh": {
					"required": {
						"sv": "Du måste mäta vänster lårs omkrets.",
						"en": "Your Left Thigh is required."
					},
					"minlength": {
						"sv": "Vänstra lårets omkrets bör vara minst 3 siffror.",
						"en": "Your Left Thigh is required to be at least 3 digits."
					},
					"maxlength": {
						"sv": "Vänstra lårets omkrets får max vara 4 siffror.",
						"en": "Your Left Thigh cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt lårmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid Left Thigh measurment, only 3-4 digits allowed."
					}
				},
				"right_thigh": {
					"required": {
						"sv": "Du måste mäta höger lårs omkrets.",
						"en": "Your Right Thigh is required."
					},
					"minlength": {
						"sv": "Högra lårets omkrets bör vara minst 3 siffror.",
						"en": "Your Right Thigh is required to be at least 3 digits."
					},
					"maxlength": {
						"sv": "Högra lårets omkrets får max vara 4 siffror.",
						"en": "Your Right Thigh cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt lårmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid Right Thigh measurment, only 3-4 digits allowed."
					}
				},
				"left_calf": {
					"required": {
						"sv": "Du måste mäta vänster vads omkrets.",
						"en": "Your Left Calf is required."
					},
					"minlength": {
						"sv": "Vänstra vadens omkrets bör vara minst 3 siffror.",
						"en": "Your Left Calf is required to be at least 2 digits."
					},
					"maxlength": {
						"sv": "Vänstra vadens omkrets får max vara 4 siffror.",
						"en": "Your Left Calf cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt vadmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid Left Calf measurment, only 2-4 digits allowed."
					}
				},
				"right_calf": {
					"required": {
						"sv": "Du måste mäta höger vads omkrets.",
						"en": "Your Right Calf is required."
					},
					"minlength": {
						"sv": "Högra vadens omkrets bör vara minst 3 siffror.",
						"en": "Your Right Calf is required to be at least 2 digits."
					},
					"maxlength": {
						"sv": "Högra vadens omkrets får max vara 4 siffror.",
						"en": "Your Right Calf cannot be larger than 4 digits."
					},
					"pattern": {
						"sv": "Icke giltigt vadmått, 3 till 4 siffror krävs.",
						"en": "That is not a valid Right Calf measurment, only 2-4 digits allowed."
					}
				}
			}
		}
	}
});